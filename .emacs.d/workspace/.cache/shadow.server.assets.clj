...
(defn ^String html-js-preload
  ...
  (let [{:keys [js-modules]}
        ...
        mods-to-link
        (into #{} (map keyword) mods)]

    (html
      (for [{:keys [module-id output-name] :as js-mod} js-modules
            :when (contains? mods-to-link module-id)]
        (html
          [:link {:rel "preload"
                  :as "script"
                  :href (str js-root "/" output-name)}]))
      )))
...
(defn ^String html-body
  ...
  (let [{:keys [js-modules]}
        ...
        mods-to-load
        (into #{} (map keyword) mods-to-load)]

    (html
      (for [{:keys [module-id output-name] :as mod} js-modules
            :when (contains? mods-to-load module-id)]
        (html
          [:script {:type "text/javascript"
                    :src (str js-root "/" output-name)}]))
      )))
