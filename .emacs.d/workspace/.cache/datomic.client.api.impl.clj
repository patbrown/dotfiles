...
(defn- serialized-require*
  [sym]
  (if-let [sr (find-var 'clojure.core/serialized-require)]
    (@sr sym)
...
(defn dynaload
  [gaid fsym]
  (try
   (serialized-require* (symbol (namespace fsym)))
...
(defn call-q
  "Call qf, a fn from Querable protocol. Unwrap io-stats and navize result."
  [arg-map qf]
  (if-let [qable (find-queryable (:args arg-map))]
    (let [qr (qf qable arg-map)]
      (if-let [io-stats (-> qr meta :io-stats)]
        {:io-stats io-stats
         :ret (navize qable nil nil qr)}
        (navize qable nil nil qr)))
